...
const COURSES = [
    { id: 1, name: "Science (PCM)", category: "Science", emoji: "🔬", tags: ["physics", "maths", "engineering", "pcm"] },
    { id: 2, name: "Science (PCB)", category: "Medical", emoji: "🧬", tags: ["biology", "medical", "doctor", "pcb"] },
    { id: 3, name: "Commerce", category: "Commerce", emoji: "📊", tags: ["accounts", "business", "finance", "ca"] },
    { id: 4, name: "Arts / Humanities", category: "Arts", emoji: "🎨", tags: ["history", "literature", "sociology", "arts"] },
    { id: 5, name: "ITI", category: "Vocational", emoji: "🔧", tags: ["trade", "technical", "electrician", "mechanic"] },
    { id: 6, name: "Diploma (Polytechnic)", category: "Technical", emoji: "⚙️", tags: ["polytechnic", "diploma", "engineering"] },
    { id: 7, name: "Fashion Designing", category: "Creative", emoji: "👗", tags: ["fashion", "design", "styling", "nift"] },
    { id: 8, name: "Nursing (GNM)", category: "Medical", emoji: "🏥", tags: ["nurse", "gnm", "healthcare", "hospital"] },
    { id: 9, name: "Agriculture", category: "Science", emoji: "🌾", tags: ["farming", "agri", "soil", "crops"] },
    { id: 10, name: "Hotel Management", category: "Vocational", emoji: "🏨", tags: ["hospitality", "chef", "hotel", "tourism"] },
    { id: 11, name: "Animation & Multimedia", category: "Creative", emoji: "🎬", tags: ["animation", "vfx", "graphics", "media"] },
    { id: 12, name: "Computer Applications (CCA)", category: "Technical", emoji: "💻", tags: ["computer", "coding", "software", "it"] },
];

const CATEGORIES = ["All", "Science", "Medical", "Commerce", "Arts", "Technical", "Vocational", "Creative"];
...
export default function After10th() {
    ...
    const [chatMessages, setChatMessages] = useState([
        {
            role: "ai",
            text: `Hi ${studentName}! 👋 I'm your AI career guide. Based on your budget (${budgetLabel}) and location (${locationLabel}), I'll suggest the best paths after 10th. What do you enjoy most?`,
        },
    ]);
    ...
    async function fetchAISearch(q) {
        ...
        try {
            const result = await askGemini(
                `
You are an AI education and career assistant for Indian students after 10th class.
...
`,
                `
Student searched for: "${q}"

Return JSON in this format:
{
  "name": "course name",
  "emoji": "one emoji",
  "category": "Science/Commerce/Arts/Medical/Technical/Vocational/Creative",
...
`
    ...
    async function fetchNearbyColleges(location) {
        try {
        ...
        `;
            ...
            return (
                data.elements?.slice(0, 8).map((college) => ({
                    name: college.tags?.name || "Unnamed College",
                    city: location,
                    state: "",
                    type: "College",
                    fees_per_year: "",
...
IMPORTANT: 
- Recommend colleges that are within the student's budget (${budgetLabel})
...
Return ONLY this JSON (no extra text):
{
  ...
  "duration": "X years",
  "eligibility": "marks/subjects needed after 10th",
  "fees": {
  ...
  "nextSteps": "Specific advice for a student in ${locationLabel} with budget ${budgetLabel} to get into this field"
        ...
        detail.topColleges = realColleges;
                        ...
                        messages: [
                            {
                                role: "system",
                                content: `
You are an expert Indian education counselor specializing in Andhra Pradesh intermediate colleges.

STRICT RULES:
1. NEVER forget the user's previously stated preferences (location, course, budget).
...
📍 **[College Name]**
- Type: Junior College / Degree College
- Courses: MPC, BiPC, etc.
...
CONTEXT TRACKING:
- Always remember: Location = [user's location], Course = [user's course], Budget = [user's budget]
- If the user says "tell me more," expand on the SAME colleges already listed.
- If the user corrects you, apologize briefly and give the corrected answer immediately.

SCOPE:
- MPC at 10th-pass level = Intermediate (11th & 12th) Junior Colleges, NOT engineering colleges.
- Do NOT suggest B.Tech or degree colleges when user asks for MPC after 10th.
When the user selects multiple interests, acknowledge EACH interest 
separately in bullet points, then suggest courses that match 
ALL or MOST of their interests combined.

Example:
- Maths + Govt job → Consider MPC → B.Sc → UPSC/State PSC
- Drawing + Earn fast → Consider Fine Arts / Animation / Design courses
-Fetch the data from the ai and give answers...based on the user requests. don't rspond unnecessarily..
`},
                            {
                                role: "user",
                                content: `
...
`,
    ...
    return (
        <div style={s.page}>
            {/* Header */}
            <div style={s.header}>
                <h1 style={{ fontSize: 28, fontWeight: 800, margin: 0 }}>📚 Courses After 10th</h1>
                ...
                <div style={s.profileBadge}>
                    <span>👤</span>
                    <span style={{ fontSize: 13, color: "#c4b8ff" }}>{studentName}</span>
            ...
            <div style={s.searchWrap}>
                <div style={{ position: "relative" }}>
                    ...
                    <input
                        ...
                        placeholder="Search any course, stream, or career field..."
                        style={s.searchInput}
                    />
                    {searching && (
                    ...
                    {CATEGORIES.map((cat) => (
                        <button
                            ...
                            style={{
                                ...
                                background: category === cat ? "#7c6af7" : "rgba(255,255,255,0.08)",
            ...
            <div style={s.layout}>
                    ...
                    {aiCard && (
                        <div style={{ marginBottom: 20 }}>
                            ...
                            <div
                                ...
                                style={{
                                    background: "linear-gradient(135deg, rgba(124,106,247,0.25), rgba(66,211,146,0.1))",
                                    border: "1.5px solid #7c6af7", borderRadius: 16, padding: "18px 22px",
                            ...
                            >
                                ...
                                <div>
                                    ...
                                    <div style={{ color: "#42d392", fontSize: 12, marginTop: 8, fontWeight: 600 }}>Click to see full details →</div>
                                </div>
                            </div>
                        </div>
                    )}
                        ...
                        {filtered.map((course) => (
                            ...
                            >
                                ...
                                <div style={{
                                    ...
                                    background: "rgba(124,106,247,0.25)", borderRadius: 10, fontSize: 11, color: "#c4b8ff",
                                }}>{course.category}</div>
                ...
                <div style={s.sidebar}>
                        ...
                        {chatMessages.map((m, i) => (
                            <div
                                ...
                                style={{
                                    ...
                                    background: m.role === "user" ? "#7c6af7" : "rgba(255,255,255,0.09)",
                        ...
                        {QUICK.map((q) => (
                            <button
                                ...
                                style={{
                                    ...
                                    color: "#c4b8ff", fontSize: 11, cursor: "pointer",
                                }}
                            >
                                {q}
                            </button>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <button
                            onClick={sendChat}
                            style={{ padding: "10px 16px", borderRadius: 12, background: "#7c6af7", border: "none", color: "#fff", cursor: "pointer", fontSize: 16 }}
                        >
                            ➤
                        </button>
                    </div>
                    ...
                    <div style={s.modal} onClick={(e) => e.stopPropagation()}>
                        ...
                        {detailLoading ? (
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 22, marginTop: 16 }}>
                                ...
                                {/* Fees */}
                                <div style={{ background: "rgba(124,106,247,0.12)", borderRadius: 14, padding: "16px 20px", border: "1px solid rgba(124,106,247,0.3)" }}>
                                    ...
                                    <div>
                                        ...
                                        <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                                            {courseDetail.careers.map((c, i) => (
                                                <span key={i} style={{
                                                    padding: "6px 16px", borderRadius: 20,
                                                    background: "rgba(124,106,247,0.22)", fontSize: 13, color: "#c4b8ff",
                                                }}>{c}</span>
                                            ))}
                                        </div>
                                    </div>
                                ...
                                {courseDetail.scholarships?.length > 0 && (
                                    <div>
                                        ...
                                        {courseDetail.scholarships.map((s, i) => (
                                            <div key={i} style={{ color: "#a9a9c8", fontSize: 13, marginBottom: 5 }}>• {s}</div>
                                        ))}
                                    </div>
                                )}
